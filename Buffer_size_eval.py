...
#
    def node_transition(self, map_list, repair_cnt=0):
        if repair_cnt == 0 and self.map == map_list:
            return -1
        if repair_cnt >= 500:
            return 500
        ...
        if not wrong_list:
            ...
            return repair_cnt
        # print(f"Repairing overflowed QPU... Attempt {repair_cnt+1}", flush=True)
        repair_cnt += 1
        ...
        wrong2 = check_validity(self.num_qpu, self.limit_qpu, modified_map_list)
        if not wrong2:
            # print(f"Repair successful after {repair_cnt+1} attempts.", flush=True)
            if modified_map_list == self.map:
                return repair_cnt+500 # If no change was made, consider it a failure to repair
            self.map = modified_map_list.copy()
            self.usage_qpu = [modified_map_list.count(i) for i in range(self.num_qpu)]
            return repair_cnt
        else:
            return self.node_transition(modified_map_list, repair_cnt )
...
#########################################################################################################################################################################################
    def PG(self, node_pre, node_next,PG_one_direct=False,trans=False):
        if trans:
            ...
            return self.node_transition(temp_map_list)
...
def total_PG(nodes,PG_one_direct=False,trans=False):
    buffer_times_oneiteration=[]
    for i in range(len(nodes)):
        if i==0: buffer_time=nodes[i].PG(None,nodes[i+1],PG_one_direct=PG_one_direct,trans=trans)
        elif i==(len(nodes)-1): buffer_time=nodes[i].PG(nodes[i-1],None,PG_one_direct=PG_one_direct,trans=trans)
        else:
            buffer_time=nodes[i].PG(nodes[i-1],nodes[i+1],PG_one_direct=PG_one_direct,trans=trans)
        buffer_times_oneiteration.append(buffer_time)
    return buffer_times_oneiteration

def total_GL(nodes,trans=False):
    buffer_times_oneiteration=[]
    for i in range(len(nodes)):
        if i==0: buffer_time=nodes[i].GL(None,nodes[i+1],trans=trans)
        elif i==(len(nodes)-1): buffer_time=nodes[i].GL(nodes[i-1],None,trans=trans)
        else:buffer_time=nodes[i].GL(nodes[i-1],nodes[i+1],trans=trans)
        buffer_times_oneiteration.append(buffer_time)
    return buffer_times_oneiteration
...
def optimize_under(file_path,num_qpu,limit_qpu,epochs,anastz=None,anastz_mode=None,print_nodes=False,compute_cost=True,randomize=False,turbulance=0,training_mode=False,PG_mode=False,FT_mode=True,period=50,GL_start_point=100,PG_one_direct=False,extra_space=1,trans=False):
    ...
    buffer_id = list(range(1, (epochs) * len(layers)+1))
    ...
    for epoch in range(GL_start_point):
        buffer_time=total_PG(nodes,PG_one_direct=PG_one_direct,trans=trans)
        Buffer_times.extend(buffer_time)
        if training_mode:
            training_cost.append(total_cost(nodes))
    for epoch in range (GL_start_point,epochs-GL_start_point):
        if epoch%cycle==0:
            buffer_time=total_GL(nodes,trans=trans)
            Buffer_times.extend(buffer_time)
        else:
            buffer_time=total_PG(nodes,PG_one_direct=PG_one_direct,trans=trans)
            Buffer_times.extend(buffer_time)
    ...
    for node in nodes:
        if max(node.map)>max_QPU:
            max_QPU=max(node.map)
    return buffer_id,Buffer_times,cost,remote_cost_list, max_QPU+1

def random_mapping(circuit_type='QFT', num_qubit=30, limit_qpu=25,num_qpu=300, seed=4321):
    random.seed(seed)
    np.random.seed(seed)
    if circuit_type == 'QFT':
        file_path = f"C:/Users/Butch/OneDrive - Stony Brook University/ICDCS_2025/qft_circuits/qft_circuit({num_qubit}qubits).txt"
    elif circuit_type == 'Rd':
        file_path = f"C:/Users/Butch/OneDrive - Stony Brook University/ICDCS_2025/random_circuits_new/{num_qubit}qubits_{num_qubit}layers.txt"
    # create the output directory if it doesn't exist
    output_dir = f"C:/Users/Butch/OneDrive - Stony Brook University/TON/NFT_perform_evals/Data_random/{circuit_type}"
...
def experiment_constant_limit(run_id,num_qpu,limit_qpu,seed,circuit_size=list(range(30,101,10)),epochs=1000,SA=False,repeat_times=1,PG_one_direct=False, Circuit_type="Rd",PG_mode=False,FT_mode=True,period=50,GL_start_point=100,extra_space=1,multi_process=False,trans=False):
    random.seed(seed)
    np.random.seed(seed)
    global calling_time
    calling_time+=1
    for repeat_time in range(repeat_times):
        ...
        for num_qubit in circuit_size:
            print(f"Running experiment for {Circuit_type} circuit with {num_qubit} qubits, limit {limit_qpu[0]},flush=True")
            if multi_process:
                if PG_mode:
                    if FT_mode:
                        output_dir = f"FT_perform_evals/parallel_processing_Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                    else:
                        output_dir = f"NFT_perform_evals/parallel_processing_Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                else:
                    if FT_mode:
                        output_dir = f"FT_perform_evals/parallel_processing_Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                    else:
                ...
                if PG_mode:
                    if FT_mode:
                        output_dir = f"FT_perform_evals/Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                    else:
                        output_dir = f"NFT_perform_evals/Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                else:
                    if FT_mode:
                        output_dir = f"FT_perform_evals/Data_{trans_type}/{Circuit_type}/Buffer/{limit_qpu[0]}_cap/circuit_size_{num_qubit}/{extra_space}_extra_space"
                    else:
            ...
            if not os.path.exists(output_file):
                pd.DataFrame(columns=[
                    'buffer_id', 'Buffer_times', 'y_local_reg', 'telegate_cost', 'teledata_cost', 'num_qpu', 'time'
                ]).to_csv(output_file, index=False)
                if Circuit_type=="Rd":
                    file_path = f"C:/Users/Butch/OneDrive - Stony Brook University/ICDCS_2025/random_circuits_new/{num_qubit}qubits_{num_qubit}layers.txt"
            ...
            buffer_id, Buffer_times,cost, remote_cost, max_QPU = optimize_under(
            ...
            print(len(buffer_id), len(Buffer_times), flush=True)
            df = pd.DataFrame({
                'buffer_id': buffer_id,
            ...
            df = df.assign(
                y_local_reg=cost,
                telegate_cost=remote_cost,
                teledata_cost=cost-remote_cost,
                num_qpu=max_QPU,
                time=elapsed_time
            )
...
if __name__ == "__main__":
    ...
    limit_QPU=[25]*300
    experiment_constant_limit(run_id=0,num_qpu=300,seed=432,circuit_size=list(range(30,101,10)),limit_qpu=limit_QPU,Circuit_type="QFT",FT_mode=False,trans=True,extra_space=6 )
...
#     circuit=Circuit(f"C:/Users/Butch/OneDrive - Stony Brook University/ICDCS_2025/random_circuits_new/{circuit_size}qubits_{circuit_size}layers.txt")
